...
Vagrant.configure(2) do |config|
  ...
  config.vm.provision :shell do |shell1|
  	shell1.inline = "mkdir -p /etc/puppet/modules;
  		puppet module install --force puppetlabs-gcc;
 		...
 		puppet module install --force darin/zypprepo"
	end
	
	config.vm.provision :shell, run: "always" do |shell|
  		shell.inline = "service iptables save
			service iptables stop
			chkconfig iptables off"
	end
    
  config.vm.provision "puppet" do |puppet|
    puppet.manifest_file  = "jenkins/jenkins.pp"
    puppet.manifests_path = "puppet"
  end
  
  config.vm.provision "puppet" do |puppet2|
    puppet2.manifest_file  = "jenkins/jenkins-job.pp"
